...
export function PayScreen({ navigation }) {
    ...
    return (
        <View className="flex-1 bg-white dark:bg-neutral-950 p-4 " onLayout={onLayoutRootView}>
            <Text className="text-2xl mb-6 text-center font-Excon_bold text-main-blue dark:text-white">Métodos de pago</Text>
            <ScrollView className="space-y-4">
                {methods.map((method) => (
                    <TouchableOpacity
                        key={method.id}
                        onPress={() => setSelectedMethod(method.id)}
                        className={`flex-row items-center justify-between p-4 border rounded-lg ${selectedMethod === method.id ? 'border-main-blue dark:border-light-blue' : 'border-gray-300 dark:border-white'
                            }`}
                    >
                        <View className="flex-row items-center">
                            <View
                                className={`w-4 h-4 mr-4 rounded-full border justify-center items-center ${selectedMethod === method.id ? 'bg-main-blue border-0 dark:bg-light-blue' : 'border-gray-300 dark:border-white'
                                    }`}
                            >
                ...
                {selectedMethod === 1 && (
                    <View className="p-4 mt-4 border rounded-lg border-gray-300 dark:border-light-blue">
                        <Text className="font-Erode_regular text-gray-800 dark:text-white text-lg">Nombre: {storeName}</Text>
                        <Text className="font-Erode_regular text-gray-800 dark:text-white text-lg">Número: {sinpeNumber}</Text>
                        <Text className="font-Erode_regular text-gray-800 dark:text-white text-lg">Detalle de compra: {generateRandomCode()}</Text>
                        <Text className="font-Erode_regular text-gray-800 dark:text-white text-lg">Total a pagar: {formatCurrency((total + deliveryFee + transportFee) / 100)}</Text>
                    </View>
                )}

                {selectedMethod === 1 && (
                    <View className="mt-4 p-4 border rounded-lg border-gray-300 dark:border-light-blue">
                        <Text className="font-Erode_regular text-gray-800 text-lg dark:text-white">Cargar el comprobante de pago:</Text>
